...
import { workerSciptData } from './worker.js'
...
type ResourceLimits = Worker extends { resourceLimits?: infer T } ? T : {};
...
const maybeFileURLToPath = (filename: string): string => (
  filename.startsWith('file:') ? fileURLToPath(new URL(filename)) : filename
)
...
class TaskInfo extends AsyncResource implements Task {
  ...
  constructor(
    ...
    // If the task is a Transferable returned by Piscina.move(),
    // then automatically add it to the transferList
    if (isMovable(task)) {
      this.transferList ??= [];
  ...
  releaseTask(): any {
    ...
    /** {@link TaskInfo}'s are used only once. */
    this.emitDestroy();

    // If an abort signal was used, remove the listener from it to prevent leaks.
...
class WorkerInfo extends AsynchronouslyCreatedResource {
  ...
  postTask(taskInfo: TaskInfo) {
    ...
    // Inform the worker that there are new messages posted, and wake it up if it is waiting for one.
...
class ThreadPool {
  ...
  _onWorkerAvailable(workerInfo: WorkerInfo): void {
    while (
      ...
      const taskInfo = this.skipQueue.shift() || this.taskQueue.shift() as TaskInfo;
  ...
  runTask(task: any, options: RunOptions): Promise<any> {
    ...
    if (signal !== null) {
      // If the AbortSignal has an aborted property and it's truthy, reject immediately.
...
class Piscina extends EventEmitterAsyncResource {
  ...
  get threads(): Worker[] {
    ...
    for (const workerInfo of this.#pool.workers) { threads.push(workerInfo.worker); }
    return threads;
  }
