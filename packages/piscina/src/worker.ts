...
async function getHandler(filename: string, name: string, fn?: string): Promise<Function | null> {
...
parentPort?.on('message', (message: StartupMessage) => {
  ...
  (async function() {
    ...
    if (filename !== null) {
      await getHandler(filename, name, fn ? fn.toString() : undefined);
    }
...
export const workerSciptData = `\
const { parentPort, MessagePort, receiveMessageOnPort, workerData } = require('worker_threads');
